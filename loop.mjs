...
attachments: [
  {
      filename: 'data.csv',
      path: 'C:\\Users\\Jain software\\Task for test\\' + 'input.csv',
      cid: 'uniq-data.csv'
  }
]
};
...
// df = pd.readCsv("huge.csv")
// df=df.reset_index(drop=True)

// for (i in df.index){
//   if (pd.isna(df['phone'][i])==True & pd.isna(df['description'][i])==False){
//     x = re.findall('^.*\s([\d\s]{6})\s.*$', df['description'][i])
//     y = re.findall('^.*(\d{5}).*$', df['description'][i])
//     if(len(x)==1 & len(y)==1){
//       df['phone'][i]=x[0]+y[0]
//     }
//   } 
// }
// df=df.drop(['placeUrl','dataId','openState','description'], axis=1)
// df=df.dropna(axis=0, subset=['phone'])
// for (i in df.index){
//   if(df['phone'][i][0]=='O' | df['phone'][i][0]=='C'){
//     df=df.drop(i)
//   }
// }
